...
height_chart = wp.line(
    [f1, f2],
    x="time", y="height", color=["Drone 1", "Drone 2"],
...
voltage_chart = wp.line(
    [f1, f2],
    x="time", y="voltage", color=["Drone 1", "Drone 2"],
    title="Battery Voltage over Time",
    xlabel="Time (UTC)", ylabel="Voltage (V)",
    theme="dark",
)
...
current_chart = wp.area(
    ...
    xlabel="Time (UTC)", ylabel="Current (A)",
    theme="watson",
)

scatter_chart = wp.scatter(
    [f1, f2],
    x="voltage", y="height", color=["Drone 1", "Drone 2"],
    title="Height vs Battery Voltage",
    xlabel="Voltage (V)", ylabel="Height (m)",
    theme="minimal",
)
...
fig.add_annotation(
    x=peak_time, y=peak_h,
    text=f"Peak {peak_h} m",
    showarrow=True, arrowhead=2,
    font=dict(color="#f6ad55"), arrowcolor="#f6ad55",
)
