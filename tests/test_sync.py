...
#     fmt = lambda t: t.strftime("%Y-%m-%dT%H:%M:%S.%f+00:00")
#     df1 = pd.DataFrame({"ts": fmt(t1), "voltage": signal[:n]})
#     # df2 observes the physical signal lag_at_500ms samples later, reported at t2
#     df2 = pd.DataFrame({"ts": fmt(t2), "voltage": signal[lag_at_500ms : lag_at_500ms + n]})
#     return df1, df2
...
# def test_sync_no_temporal_overlap_raises():
#     df1, df2 = _make_logs()
#     # Shift df2 far into the future so there's no overlap
#     df2["ts"] = (pd.to_datetime(df2["ts"], utc=True) + pd.Timedelta(days=365)).dt.strftime(
#         "%Y-%m-%dT%H:%M:%S.%f+00:00"
#     )
#     with pytest.raises(NoTemporalOverlapError):
#         wp.sync(df1, df2, on="voltage", time1="ts", time2="ts")
...
#     fmt = lambda t: t.strftime("%Y-%m-%dT%H:%M:%S.%f+00:00")
#     df1 = pd.DataFrame({"ts": fmt(t), "v": rng.normal(0, 1, n)})
#     df2 = pd.DataFrame({"ts": fmt(t), "v": rng.normal(0, 1, n)})
